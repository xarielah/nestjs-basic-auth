...
export class VerificationTokenService {
  ...
  /**
   * Gets a payload and return a signed JWT, with a 7 days expiration time.
   * @param {TokenPayload} payload
   * @returns token
   */
  public async createToken(payload: TokenPayload): Promise<string> {
    return this.tokenService.sign(payload, '1s');
  }
  ...
  private async removeToken(userId: string): Promise<boolean> {
   ...
   * @returns boolean
   */
  public async verifyAndDeleteVerificationToken(
    userId: string,
    token: string,
  ): Promise<boolean> {
    try {
